      ...
      <UserInfoArea onSubmit={handleSubmit(modifyDataHandler)}>
        <div className="fcc" style={{ justifyContent: "flex-start" }}>
          ...
          <UserInfo bgColor={flag ? "inherit" : "white"}>
            <li>
              <p>이메일</p>
              <input {...register("email")} disabled defaultValue={email} />
            </li>
            <li>
              <p>비밀번호 찾기 질문</p>
              <input {...register("answer")} disabled={flag} defaultValue={answer} />
            </li>
            <li>
              <p>닉네임</p>
              <input {...register("nickname")} disabled={flag} defaultValue={nickname} />
            </li>
            <li style={{ opacity: flag ? 0 : 1 }}>
              <p>새 비밀번호</p>
              <input {...register("password")} type="password" placeholder="변경이 필요할때만 기입!" disabled={flag} />
            </li>
          </UserInfo>
        </div>
        <BtnArea className="fcc">
          <InputBtn onClick={toggleModify}>{flag ? "수정" : "취소"}</InputBtn>
          <InputBtn onClick={flag ? deleteUserHandler : null} disabled={isSubmitting}>
            {flag ? "탈퇴" : "완료"}
          </InputBtn>
        </BtnArea>
      </UserInfoArea>
